...
def _percent_print(demographic_string, total_nodes, attribute_counts):
    """
    Pretty print feature for percentages of attributes.
    """
    print(
        '\t{0:11.11}:\t{1}\t{2:5.5}%'.format(
            demographic_string,
            attribute_counts[demographic_string],
            attribute_counts[demographic_string] /
            total_nodes))
...
def _print_demo_sector(total_nodes, attribute_counts):
    """
    Pretty print feature for demographic section (gender, major, activities).
    """
    print('-' * 60)
    for attribute, count in sorted(
            attribute_counts.items(), key=lambda x: -x[1]):
...
def calculate_node_percentages(graph):
    """
    Calculates the demographic make-up of the graph and reports.
    """
